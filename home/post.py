  ...
  "universe_domain": "googleapis.com"
    ...
    try:
        ...
        if post.typename == "GraphVideo":
            furl=upload_file(post.video_url,username+".mp4")
            vthumb=upload_file(post.url,username+"th.png")
            videothumb.append(vthumb)

            videopost.append(furl)
            return Response({"account_type":False,"videourls":videopost,"videothumb": videothumb,"imageurls": imagepost, "username":username ,"type":"video"}) 

        elif post.typename == "GraphImage":

            furl=upload_file(post.url,username +".jpg")
            ...
            for media in post.get_sidecar_nodes():             
             if(media.is_video == True):
                ...
                imagepost.append(furl)
                count+=1
            return Response({"account_type":False,"videourls":videopost, "videothumb": videothumb,"imageurls": imagepost, "username":username,"type":"sidecar"}) 
